...
use std::env;
use rustfmt::Input;
use proc_macro2::{TokenStream, Punct, Spacing, Delimiter, Group};
...
impl ToTokens for Document {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        field(&mut fields, "participants", |tokens| self.participants.to_tokens(tokens));
...
impl ToTokens for DocumentParticipant {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let mut fields = TokenStream::new();
        field(&mut fields, "cursor_pos", |tokens| self.cursor_pos.to_tokens(tokens));
...
impl ToTokens for Edit {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let mut fields = TokenStream::new();
        field(&mut fields, "author", |tokens| self.author.to_tokens(tokens));
...
fn write_test(mut output: &File, name: &str, mut file: File) {
    ...
    match config {
        TestConfig::Apply {initial, expected, events} => {
            write!(output, "{}\n", quote! {
                #[test]
                fn #name() {
                    let mut doc = #initial;
